...
/* open alsa backend */
void AlsaDriver::open(Audio::Parameters* pParams)
{
...
void AlsaDriver::close()
{
    ...
    if (snd_pcm_close(data_->device) < 0) {
        throw Audio::AudioError("Unable to close the Audio Interface.");
    }
}
...
void AlsaDriver::setup()
{
    ...
    if (error < 0) {
        throw Audio::AudioError("Unable to setup the Audio Interface.");
    }

    // prepare the device for playing
    if (snd_pcm_prepare(data_->device) < 0) {
        throw Audio::AudioError("Unable to prepare the Audio Interface.");
    }
}
